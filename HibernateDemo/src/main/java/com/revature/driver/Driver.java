...
public class Driver {
	...
	public static void main(String[] args) {
		...
		// Bears with name starting with "G"
		// Bats with wingspans between 50 and 100
		System.out.println("Bears starting with G, bat wingspans 50-100");
		criteriaChallenge();

		// Projections
		System.out.println("Total wingspan of bats: ");
		getTotalWingSpan();
		System.out.println("Count of bears:");
		getTotalBears();
	}

	static List getTotalWingSpan() {
		Session s = HibernateUtil.getSession();

		Criteria c = s.createCriteria(Bat.class);
		List list = c.setProjection(Projections.projectionList().add(Projections.sum("wingspan"))).list();
		if (list == null) {
			return null;
		}
	...
	static List getTotalBears() {
		Session s = HibernateUtil.getSession();

		Criteria c = s.createCriteria(Bear.class);
		List list = c.setProjection(Projections.projectionList().add(Projections.count("bearType"))).list();
		if (list == null) {
			return null;
		}
	...
	static void criteriaChallenge() {
		Session s = HibernateUtil.getSession();
		Transaction tx = s.beginTransaction();

		List<Bear> bears = s.createCriteria(Bear.class).add(Restrictions.like("name", "G%")).list();

		System.out.println(bears);
		List<Bat> bats = s.createCriteria(Bat.class).add(Restrictions.between("wingspan", 50, 100)).list();
